...
(async function exec() {
   ...
   /* Comment on 05March2022 const zip6fab = new (require('adm-zip'));
    ...
    await copy('./build/packagefiles/pkg_commercelab_shop.xml', './build/output/package/pkg_commercelab_shop.xml');
    await copy('./build/packagefiles/script.php', './build/output/package/script.php');

    if (!(await exists('./dist/component'))) {
        await mkdir('./dist/component');
    }
    ...
    zip.writeZip(`dist/component/pkg_commercelab_shop_${version}.zip`);
