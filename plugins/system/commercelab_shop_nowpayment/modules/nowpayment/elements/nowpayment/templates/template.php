...
<?= $el($props, $attrs) ?>
    ...
    <div id="<?= $id; ?>">
        ...
        <div id="NOWpayment_modal_<?= $id; ?>" class="uk-modal-full" uk-modal>
            ...
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-text-center">{{ cartTotal }}</h3>
                </div>

                <div class="uk-modal-body" uk-overflow-auto>

                    <div class="uk-grid-divider" uk-grid uk-height-match="target: .uk-card">

                        <div class="uk-width-1-1">

                            <div class="uk-grid-collapse uk-child-width-1-1 uk-child-width-1-2@s uk-child-width-1-3@m uk-grid-stack" uk-grid>

                                <div v-for="currency in currencies" class="uk-padding-small uk-card uk-card-default uk-card-body">

                                    <div class="uk-grid-collapse" uk-grid>
                                        <div class="uk-width-1-4 uk-padding@s uk-padding-small uk-padding-remove-top">
                                            <img loading="lazy" class="uk-animation uk-animation-slide-bottom-small uk-width-full" :src="'https://nowpayments.io' + currency.logo_url">
                                        </div>
                                        <div class="uk-width-3-4">
                                            <h5 class="uk-margin-small">{{ currency.name }}</h5>
                                            <h6 class="uk-margin-remove">{{ currency.code }}</h6>
                                        </div>
                                        <div class="uk-width-1-1 uk-margin-small-top">
                                            <div uk-grid>
                                                ...
                                                <div class="uk-width-auto uk-float-right">
                                                    ...
                                                    <a v-if="!currency.calculated && !currency.calculating" href="javascript:void(0);" 
                                                        @click="calculatePrice(currency.code)" class="uk-button uk-button-small uk-button-default uk-float-right"
                                                        :class="{'uk-disabled':currency.disabled}"
                                                    >
                                                        Calculate
                                                    </a>

                                                    <span v-if="currency.calculating" style="width: 20px; height: 20px;" uk-spinner></span>

                                                    <a v-if="currency.calculated && !currency.calculating" href="javascript:void(0);" 
                                                        @click="initPayment(currency.code, currency.calculated)" class="uk-button uk-button-small uk-button-primary uk-float-right"
                                                        :class="{'uk-disabled':currency.selling}"
                                                    >
                                                        <span v-if="currency.selling" class="uk-margin-small-right" style="width: 20px; height: 20px;" uk-spinner></span>
                                                        <span>
                                                            Buy
                                                        </span>
                                                    </a>
        ...
        <span v-if="loading" style="width: 20px; height: 20px;" uk-spinner></span>
    ...
    <script>
        const <?= $id; ?> = {
            ...
            created() {
                ...
                emitter.on("total_cart_updated", this.totalPriceUpdated);
            },
            ...
            methods: {
                ...
                async totalPriceUpdated(total) {
                    this.cartTotal = total;
                    this.currencies.forEach(currency => {
                        currency.calculating = currency.calculated = false
                    });

                },

                async calculatePrice(currency_code) {
                    ...
                    this.currencies.forEach(currency => {
                        if (currency.code == currency_code)
                        {
                            ...
                            selected_currency.calculating = true;
                    ...
                    const send     = JSON.parse(JSON.stringify(this.ajax_headers));
                    const request  = await fetch(Joomla_cls.uri_base + 'index.php?option=com_ajax&plugin=Commercelab_shop_nowpayment&task=calculateCrypto&format=json&crypto_currency=' + currency_code, send);
                    const response = await request.json();

                    if (response.success)
                    {

                        selected_currency.calculating = false;
                        selected_currency.calculated  = response.data[0];
                        return true;
                    }
                    else
                    {
                        ...
                        selected_currency.calculating = false;
                        selected_currency.disabled    = true;

                        UIkit.notification({
                            message: 'Failed to get estimate amount, please use a different currency',
                            status: 'danger',
                            pos: 'top-center',
                            timeout: 5000
                        });
                ...
                async getCurrencies() {
                    ...
                    {
                        UIkit.modal('#NOWpayment_modal_<?= $id; ?>').show();
                        return true;
                    }
                    ...
                    const send     = JSON.parse(JSON.stringify(this.ajax_headers));
                    const request  = await fetch(Joomla_cls.uri_base + 'index.php?option=com_ajax&plugin=Commercelab_shop_nowpayment&task=getCurrencies&format=json', send);
                    const response = await request.json();
