...
return [
	...
	'transforms' => [
		...
		'render' => function ($node, array $params) {
			...
			$paymentPlugin = PluginHelper::getPlugin('commercelab_shop_payment', 'stripepayment');
			...
			if ($paymentPluginParams->get('live'))
			{
				if (!$paymentPluginParams->get('live_public_key') || !$paymentPluginParams->get('live_private_key')) {
					...
					Factory::getApplication()->enqueueMessage('Please enter your Stripe Private Key and Endpoint secret in the Stripe Payment - CommerceLab Shop plugin.', 'warning');
					return false;
				}
