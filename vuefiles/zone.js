...
const p2s_zone_form = {
    ...
    mounted() {
        console.log(this.form.jform_country);
    },
    computed: {},
    methods: {
        ...
        async saveItem() {
            ...
            const params = {
                ...
                inherit_taxrate: (this.form.jform_inherit_taxrate) ? 1 : 0,
                published: (this.form.jform_published) ? 1 : 0
            };

            if (!params.inherit_taxrate)
            {
                params.taxrate         = this.form.jform_taxrate;
                params.taxrate_reduced = this.form.jform_taxrate_reduced;
                params.taxrate_extra   = this.form.jform_taxrate_extra;
            }

            const request = await fetch("index.php?option=com_ajax&plugin=commercelab_shop_ajaxhelper&method=post&task=task&type=zones.save&format=raw", {
                method: 'POST',
                mode: 'cors',
                cache: 'no-cache',
            ...
            if (response.success)
            {
                ...
                if (this.andClose) {
                    ...
                    window.location.href = 'index.php?option=com_commercelab_shop&view=zones';
                } else {
