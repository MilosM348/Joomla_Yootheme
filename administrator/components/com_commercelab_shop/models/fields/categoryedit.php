 ...
 * @since 2.0
 */
class JFormFieldCategoryEdit extends JFormField
{

	public function getLabel()
	{
		// <div v-if="!form.jform_title" class="uk-text-small uk-text-warning">'.Text::_('COM_COMMERCELAB_SHOP_OPTIONS_TEMPLATES_TITLE_BEFORE_CHANGE_CAT').'</div>
	...
	protected function getInput()
	{
		...
		$edit_filter_category_selector = Route::_('index.php?option=com_categories&view=categories&layout=modal&tmpl=component&function=productCategory&' . Session::getFormToken() . '=1');

		$html = [];
		$html[] = '<div uk-lightbox data-type="iframe" id="edit-category-selector" class="uk-position-relative">';
			$html[] = '<a href="' . $edit_filter_category_selector . '" class="uk-form-icon uk-form-icon-flip" uk-icon="icon: file-edit"></a>';
			$html[] = '<input type="text" data-stored-value="'.$this->value.'" :value="productCategoryName" >';
		$html[] = '<p-inputnumber type="hidden" name="' . $this->name . '" v-model="form.' . $this->id . '" id="' . $this->id . '">';
		$html[] = '</p-inputnumber> ';
		$html[] = '</div>';
